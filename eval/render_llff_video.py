...
from configs.local_setting import OUT_DIR, LOG_DIR, EVAL_CONFIG, DATA_DIR, VIDEO_CONFIG
from eval.init_eval import init_eval, rearrange_args_for_eval
...
#                  **kwargs):
#
#         self.folder_path = DATA_DIR / 'nerf_llff_data'
...
#             scene_path = self.folder_path / scene
#             _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=4)
#             near_depth = np.min(bds)
#             far_depth = np.max(bds)
#             intrinsics, c2w_mats = batch_parse_llff_poses(poses)
